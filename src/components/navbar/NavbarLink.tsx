            ...
            <AnimatePresence>
                { menuExpanded === title ? (
                    <motion.ul
                        ...
                        className={`flex flex-col items-start lg:items-end gap-2 ${ isMobile ? "pl-4 border-l border-gray-300" : "absolute top-full right-0 text-end" }`}
                    >
                        {links.map((link, i) => (
                            <motion.li
                                initial={{ x: 50, opacity: 0 }}
                            ...
                            >
                                <Link
                                    href={link.href}
                                    className={`${isHome ? "text-white/90 hover:bg-white/25" : "text-black/90"} w-fit uppercase font-medium tracking-widest whitespace-nowrap transition-150 block`}
                                >
                                    {link.title}
                                </Link>
                            </motion.li>
