...
export async function generateStaticParams() {
    return getCollaborations().map((collab) => ({
        collaboration: collab.id,
    }));
}

export default function CollaborationPage({ params }: Props) {
    const { collaboration } = params;
    const found = collections.find(c => c.id === collaboration && c.collaboration);
    if (!found) notFound();
    ...
    return (
        <main className="my-24 lg:my-48 w-full grow flex flex-col items-center font-truetypewritter">
            <h1 className="sr-only">Colaboración Mercedes Costal {found.title}</h1>
            <section className="w-full px-4 lg:px-12 flex flex-col justify-center items-center gap-8 lg:gap-4">
                {found.video ? (
                    <CollectionVideo title={found.title} video={found.video} />
                ) : (
                    <div className="w-full aspect-video relative">
                        ...
                        <div className="size-full bg-black/20 absolute top-0 left-0 z-30" />
            ...
            {found.murales.length > 0 &&
                <section className="mt-12 lg:mt-24 w-full px-4 lg:px-12 grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-6 lg:gap-y-16">
                    {sortMurales(found.murales).map((mural, index) => <MuralCardNew mural={mural} index={index} key={mural.id} />)}
                </section>
            }
            <CTA />
        </main>
